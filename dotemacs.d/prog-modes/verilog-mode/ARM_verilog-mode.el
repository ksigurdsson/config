...
;;
;; $Id: verilog-mode.el,v 3.50 2000/11/03 22:50:42 mac Exp $
...
(defconst verilog-mode-version "$$Revision: 3.50 $$"
...
(defcustom verilog-linter "bsub -I -q inter -R \"$LSF_SPYGLASSVERILOG_RES\" spyglass -batch -I /home/ksigurds/work/SAFE/spyglass/ -verilog %s  +libext+.v -report armemacs -reportfile stdout"
...
(defcustom verilog-tool (concat (eval verilog-linter) " -policy=lint -fast ; rm -rf spyglass*")
...
(defcustom verilog-auto-sense-defines-constant t
  ...
  '("Verilog"
    ("Choose Compilation Action"
     ("Spyglass"
      ["Fast Lint"
       (progn
	 (setq verilog-tool (concat (eval verilog-linter) " -policy=lint -fast"))
	 (setq verilog-which-tool 1)
	 (customize-set-variable 'verilog-tool (concat (eval verilog-linter) " -policy=lint -fast"))
	 (verilog-set-compile-command))
       :style radio
       :selected (= verilog-which-tool 1)]
      ["Standard Lint"
       (progn
	 (setq verilog-tool (concat (eval verilog-linter) " -policy=lint"))
	 (setq verilog-which-tool 2)
	 (customize-set-variable 'verilog-tool (concat (eval verilog-linter) " -policy=lint"))
	 (verilog-set-compile-command))
       :style radio
       :selected (= verilog-which-tool 2)]
      ["Strict Lint"
       (progn
	 (setq verilog-tool (concat (eval verilog-linter) " -policy=lint -strict"))
	 (setq verilog-which-tool 3)
	 (customize-set-variable 'verilog-tool (concat (eval verilog-linter) " -policy=lint -strict"))
	 (verilog-set-compile-command))
       :style radio
       :selected (= verilog-which-tool 3)]
      ["OpenMore"
       (progn
	 (setq verilog-tool (concat (eval verilog-linter) " -policy=openmore"))
	 (setq verilog-which-tool 4)
	 (customize-set-variable 'verilog-tool (concat (eval verilog-linter) " -policy=openmore"))
	 (verilog-set-compile-command))
       :style radio
       :selected (= verilog-which-tool 4)]
      ["Timing"
       (progn
	 (setq verilog-tool (concat (eval verilog-linter) " -policy=timing"))
	 (setq verilog-which-tool 5)
	 (customize-set-variable 'verilog-tool (concat (eval verilog-linter) " -policy=timing"))
	 (verilog-set-compile-command))
       :style radio
       :selected (= verilog-which-tool 5)]
      ["Area"
       (progn
	 (setq verilog-tool (concat (eval verilog-linter) " -policy=area"))
	 (setq verilog-which-tool 6)
	 (customize-set-variable 'verilog-tool (concat (eval verilog-linter) " -policy=area"))
	 (verilog-set-compile-command))
       :style radio
       :selected (= verilog-which-tool 6)]
      )
     ["Coverage"
      (progn
	(setq verilog-tool 'verilog-coverage)
	(setq verilog-which-tool 7)
	(customize-set-variable 'verilog-tool verilog-coverage)
	(verilog-set-compile-command))
      :style radio
      :selected (= verilog-which-tool 7)]
     ["Simulator"
      (progn
	(setq verilog-tool 'verilog-simulator)
	(setq verilog-which-tool 8)
	(customize-set-variable 'verilog-tool verilog-simulator)
	(verilog-set-compile-command))
      :style radio
      :selected (= verilog-which-tool 8)]
     ["Compiler"
      (progn
	(setq verilog-tool 'verilog-compiler)
	(setq verilog-which-tool 9)
	(customize-set-variable 'verilog-tool verilog-compiler)
	(verilog-set-compile-command))
      :style radio
      :selected (= verilog-which-tool 9)]
     )
    ("Move"
